...
static void user_item_init (UserItem *self) {
	...
	self->iconw = gtk_image_new_from_resource ("/github/xverizex/nem-desktop/human_default.svg");
	gtk_widget_set_name (self->iconw, "item");

	self->statusw = gtk_image_new_from_resource ("/github/xverizex/nem-desktop/status_offline.svg");
		...
		case PROP_STATUS:
			...
			if (self->status) {
				if (self->statusw) {
					...
					self->statusw = gtk_image_new_from_resource ("/github/xverizex/nem-desktop/status_online.svg");
					g_object_set (self->statusw, "halign", GTK_ALIGN_END, "hexpand", TRUE, NULL);
					gtk_widget_set_name (self->statusw, "item");
				}
				gtk_box_append (GTK_BOX (self->box), self->statusw);
